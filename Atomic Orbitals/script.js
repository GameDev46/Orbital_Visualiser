...
let scene, camera, renderer, clock, keyboard, controls, orbitalMesh, timer, stats, enableSpin, spinVelocity;
...
function init() {
	...
	const probabilityInput = settingsFolder.add(params, "pointDensity", 0, 5, 0.01);
	probabilityInput.onChange((value) => {
		scene.remove(orbitalMesh);
		orbitalMesh = generateOrbital(value);
	});

	// Create show inside option
	const showInsideInput = settingsFolder.add(params, "viewInside");
	showInsideInput.onChange((value) => {
		orbitalMesh.material.uniforms.showInside.value = value;
	}).listen();

	// Create enable spin option
	const enableSpinInput = settingsFolder.add(params, "enableSpin");
	enableSpinInput.onChange((value) => {
		enableSpin = value;
	}).listen();

	// Create enable spin speed option
	const spinVelocityOption = settingsFolder.add(params, "spinVelocity", 0, 10, 0.01);
	spinVelocityOption.onChange((value) => {
		spinVelocity = value;
	}).listen();

	// Create grid option
	const gridInput = settingsFolder.add(params, "showGrid");
	...
	const backgroundInput = settingsFolder.addColor(params, "background");
...
function generateOrbital(probabilityMultiplier = 1) {
    const numPoints = 300000 * (n + l);
    ...
    for (let i = 0; i < numPoints; i++) {
		...
		let scaledProbability = probability * probabilityMultiplier;
		scaledProbability *= 5 * (l * l + n);
		let densityScaledProbability = scaledProbability * Math.max(0.1, Math.pow(r, 2));
